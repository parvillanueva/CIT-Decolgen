CKEDITOR.plugins.add('youtube', {
    ...
    init: function (editor) {
        var cmd = editor.addCommand('open_youtube', { exec: open_youtube });
        editor.ui.addButton('Youtube', {
            label: 'Youtube',
            command: 'open_youtube',
            icon: CKEDITOR.plugins.getPath('youtube') + 'youtube.png'
        });
    }
});

var editor = null;
function open_youtube(e) {
    $('#youtube_modal').modal("show");
    editor = e;
}

element.click("#youtube_insert", function(){
	var html = "";
	var url = $("#youtube_url").val();
	var responsive = $("#youtube_responsive").val();
	var autoplay = $("#youtube_autoplay").val();
	var width = "560";
	var height = "315";
	...
	if(match_youtube_url(url)){
		...
		if($("#youtube_responsive").is(':checked')){
			html += '<div style="position: relative;padding-bottom: 56.25%;padding-top: 25px;height: 0;">';
			html += '<iframe style="position: absolute;top: 0;left: 0;width: 100%;height: 100%;" src="https://www.youtube.com/embed/'+youtube_parser(url)+'" frameborder="0" allow="'+allow+'" allowfullscreen></iframe>';
			html += '</div>';
		} else {
			width = $("#youtube_width").val();
			height = $("#youtube_height").val();
			html += '<iframe width="'+width+'" height="'+height+'" src="https://www.youtube.com/embed/'+youtube_parser(url)+'" frameborder="0" allow="'+allow+'" allowfullscreen></iframe>';
		}
	} else {
	...
	$("#youtube_url").val("");
	$("#youtube_responsive").val(0);
	$("#youtube_autoplay").val(0);
