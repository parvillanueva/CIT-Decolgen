...
$(document).on('change', '#menu_template', function(e){
	var url = content_management + '/preference/navigation/template';
...
function get_roles(){
	...
	AJAX.select.select("id,name,status");
...
$(document).on('click', '#btn_save', function(e){
	...
	if(validate.standard('menu')){
		if(is_exists('cms_menu', 'menu_name', $('#menu_name').val(), 'menu_status') != 0)
		{
        	...
        	modal.standard(confirm_save, function(result){
				if(result){
					propag++;
					modal.loading(true);
					setTimeout(function(){
...
function group(){	
	var url = content_management + '/preference/navigation/save';	
	var data = {
		menu : $('#menu_name').val(),
		icon : $('#icon').val(),
		template : "group",
	...
	aJax.post(url,data,function(result){
		...
		menu_roles(obj);
		modal.alert(save_success, function(){
			location.href = content_management + '/cms_menu/menu/' + menu_id;
		});
	});
}
...
function menu_roles(menu_id){
	...
	for (var i = 0; i < counter; i++) {
		...
		AJAX.insert.params("role_id", $('.role_id_'+i+'').attr('data-id'));
		AJAX.insert.params("menu_id", menu_id);
		AJAX.insert.params("menu_role_read", $('.read_role_'+i+'').val());
		AJAX.insert.params("menu_role_write", $('.write_role_'+i+'').val());
		AJAX.insert.params("menu_role_delete", $('.delete_role_'+i+'').val());
